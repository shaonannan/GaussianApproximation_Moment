...
Net_settings = {'hyp_num': 1,
                ...
                'yhyp':1,
                'xn':2,
                'yn':2, # 25 subpopulation each 20 cells 20 * 25 = 500 cells per hyp!
                'dxx_hyp':500,
                'nmax': 0,
                'Final_time':800,
                'dt':0.1,
...
def input_signal_stream(active_resp,t_properties,Net_settings):
    ...
    for layer, celltype in itertools.product(CG_range, ['e', 'i']):  
        for iphase in range(NPHA):
            tmp_Input = 1.9*np.squeeze(active_resp[layer,iphase,:])
...
g_norm = {'short_range_exc_exc':1.25/5.0/NE_source/Net_settings['xn']/Net_settings['yn']/Fun_map_settings['phase_num'] ,#0.24/NE_source,
           'short_range_exc_inh':1.30/5.0/NE_source/Net_settings['xn']/Net_settings['yn']/Fun_map_settings['phase_num'] ,#0.33/NE_source,
           'short_range_inh_exc':-1.32/5.0/NI_source/Net_settings['xn']/Net_settings['yn']/Fun_map_settings['phase_num'] ,#-0.44/NI_source,
           'short_range_inh_inh':-0.65/5.0/NI_source/Net_settings['xn']/Net_settings['yn']/Fun_map_settings['phase_num'] ,#-0.09/NI_source,
           
           'long_range_exc_exc_fast':0.84/5.0/NE_source/AmpHyp * AmpOri/Fun_map_settings['phase_num'] ,#0.364/NE_source,#0.426/NE_source,#0.24/NE_source,
           'long_range_exc_inh_fast':0.8/5.02/NE_source/AmpHyp * AmpOri/Fun_map_settings['phase_num'] ,##0.396/NE_source,#0.33/NE_source,
           
           'long_range_exc_exc':0.664/5.0/NE_source/AmpHyp * AmpOri * 6.64/Fun_map_settings['phase_num'] ,#2.46,#1.64,#2.56,#2.46,#1.26,#1.28,
           'long_range_exc_inh':0.664/5.0/NE_source/AmpHyp * AmpOri * 6.62/Fun_map_settings['phase_num'] }#2.86}#1.96}#2.86}#2.86}#1.64}#1.96}
...
(mEbin_ra,mIbin_ra,xEbin_ra,xIbin_ra,VEavgbin_ra,VIavgbin_ra,VEstdbin_ra,VIstdbin_ra,nEbin_ra,nIbin_ra) = simulation.update(t0=t0, dt=dt, tf=tf)

import time
import scipy.io as scio 
ISOTIMEFORMAT='%Y%m%d%H%M%S'
filename=str(time.strftime(ISOTIMEFORMAT)) + '_large.mat'
scio.savemat(filename,{'mEbin_ra':mEbin_ra,'mIbin_ra':mIbin_ra,'xEbin_ra':xEbin_ra,'xIbin_ra':xIbin_ra,'VEavgbin_ra':VEavgbin_ra,'VIavgbin_ra':VIavgbin_ra,'VEstdbin_ra':VEstdbin_ra,'VIstdbin_ra':VIstdbin_ra,'nEbin_ra':nEbin_ra,'nIbin_ra':nIbin_ra}) 
