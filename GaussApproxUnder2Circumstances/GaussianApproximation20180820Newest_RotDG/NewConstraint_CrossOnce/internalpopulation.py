...
class RecurrentPopulation(object):
    ...
    def update_fp_moment4_Gauss(self):
        ...
        v1n = v1 + dtgL*( - (v1-vs)) # -fr/gL
        v2n = v2 + dtgL*( - 2.0*(v2-vs*v1-0.5*ds)) # -fr/gL
        v3n = v3 + dtgL*( - 3.0*(v3-vs*v2-ds*v1)) # -fr/gL
        v4n = v4 + dtgL*( - 4.0*(v4-vs*v3-1.5*ds*v2)) #-fr/gL
    ...
    def update(self):
        ...
        if self.USUALorGauss < 1: 
            ...
            firing_rate  = gL*rhov[-1]*(-1.0+vs)-gL*ds/2.0*rhov[-1]*(-(1.0-M1)/M2)  # ignore firing rate

            if firing_rate <0:
                # re-locate rhov/rhoEQ/
                # self.update_fp_moment4() # self update_fp_moment4 will increase M1,M2 at each iteration
                self.update_trad_rhoEQ()
                self.USUALorGauss = 1 
        ...
        if self.USUALorGauss<1:
            ...
            if self.USUALorMFE:
                ...
                if self.USUALorGauss < 1:
                    ...
                    # self.update_fp_moment4() # have already
                    self.update_trad_rhoEQ()
