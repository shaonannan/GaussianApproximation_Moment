...
def input_signal_stream(active_resp,t_properties,Net_settings):
    ...
    for layer, celltype in itertools.product(CG_range, ['e', 'i']):  
        tmp_Input = 0.9*np.squeeze(active_resp[layer])
        ExtInp = External_stimuli_dict + tmp_Input[:ntt]
        background_population_dict[layer,celltype] = ExternalPopulation(ExtInp,dt, record=False)
...
(mEbin_ra,mIbin_ra,xEbin_ra,xIbin_ra,VEavg_ra,VIavg_ra,VEstd_ra,VIstd_ra,rEbin_ra,rIbin_ra,P_MFE_ra,P_MFEbin_ra,VEavgbin_ra,VIavgbin_ra,VEstdbin_ra,VIstdbin_ra,nEbin_ra,nIbin_ra) = simulation.update(t0=t0, dt=dt, tf=tf)

import time
import scipy.io as scio 
ISOTIMEFORMAT='%Y%m%d%H%M%S'
filename=str(time.strftime(ISOTIMEFORMAT)) + '_large.mat'
scio.savemat(filename,{'mEbin_ra':mEbin_ra,'mIbin_ra':mIbin_ra,'xEbin_ra':xEbin_ra,'xIbin_ra':xIbin_ra,'VEavg_ra':VEavg_ra,'VIavg_ra':VIavg_ra,'VEstd_ra':VEstd_ra,'VIstd_ra':VIstd_ra,'rEbin_ra':rEbin_ra,'rIbin_ra':rIbin_ra,'P_MFE_ra':P_MFE_ra,'P_MFEbin_ra':P_MFEbin_ra,'nEbin_ra':nEbin_ra,'nIbin_ra':nIbin_ra}) 
