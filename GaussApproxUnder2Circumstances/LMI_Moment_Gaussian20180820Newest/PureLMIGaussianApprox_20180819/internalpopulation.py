...
class RecurrentPopulation(object):
    ...
    def update_GauApprox_rhoEQ(self):
        ...
        try:
            Rv[indp[0][:]] = np.exp(-np.square(V[indp[0][:]]-Vs)/2.0/D)
    ...
    def update(self):
        ...
        # EACH TIME ENTER AN ITERATION, WE SHOULD RESET USUALorGauss to initial ZERO
        # TO ENSURE THAT TRADITIONAL RHOEQ WOULD DRAG IT OUT FROM THE BAD SINGULAR POINT
        self.USUALorGauss = 0
        if self.USUALorMFE:
            rhov   = rhoEQ.copy()
