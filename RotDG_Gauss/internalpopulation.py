...
class RecurrentPopulation(object):
    ...
    def update_fp_moment4_Gauss(self):
        ...
        v1n = v1 + dtgL*( -fr/gL - (v1-vs)) # - (v1-vs)) #
        v2n = v2 + dtgL*( -fr/gL - 2.0*(v2-vs*v1-0.5*ds)) #- 2.0*(v2-vs*v1-0.5*ds)) #
        v3n = v3 + dtgL*( -fr/gL - 3.0*(v3-vs*v2-ds*v1)) # - 3.0*(v3-vs*v2-ds*v1)) #
        v4n = v4 + dtgL*( -fr/gL - 4.0*(v4-vs*v3-1.5*ds*v2)) # - 4.0*(v4-vs*v3-1.5*ds*v2)) # 
...
#                        VI = np.append(VI,c.V_sample_post)
    ...
    def update(self):
        if self.USUALorMFE:
            ...
            self.update_total_fpmu_dict()
            self.update_rhoEQ()
        if self.USUALorGauss < 1:   # testing whether firing_rate also fit the requirement
            vs   = self.total_fp_vslave
            ds   = self.total_fp_sigv
            ...
            firing_rate  = -gL*ds/2.0*rhov[-1]*(-(1.0-M1)/M2) # gL*rhov[-1]*(-1.0+vs) # Original Gaussian Approximation

            if firing_rate <0:
                # re-locate rhov/rhoEQ/
                self.update_trad_rhoEQ()
                self.USUALorGauss = 1 
        ...
        if self.USUALorGauss<1:
            ...
            if self.USUALorMFE:
                ...
                ''' MFE didn't recalculate firing rate based on this equation, only base on LE'''
                firing_rate  = -gL*ds/2.0*rhov[-1]*(-(1.0-M1)/M2) # gL*rhov[-1]*(-1.0+vs)-gL*ds/2.0*rhov[-1]*(-(1.0-M1)/M2)
                if firing_rate <0:
                    self.USUALorGauss = 1
                ...
                if self.USUALorGauss < 1:
                    ...
                    # else, donot record and throw into next step
                else:
                    self.update_trad_rhoEQ()
                    # re-calculate rhoEQ and throw into next step
        ...
        if self.USUALorGauss>0:
            # Here, when M1>vT, we use USUAL ME(Maximum Entropy) algorithm to do the evolution
            vs   = self.total_fp_vslave
            ds   = self.total_fp_sigv
            La0  = self.La0
            fin  = self.fin
            ...
            if self.USUALorMFE:
                ...
                if np.isnan(firing_rate)|np.isnan(np.sum(rhov))|\
                    np.isinf(firing_rate)|np.isinf(np.sum(rhov))|\
